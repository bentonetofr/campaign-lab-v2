-- ============================================================
-- Vorterium — Manter o menor (keep_lowest) na rolagem de dados
-- Migration: 20240130000000_dice_keep_lowest.sql
-- Aplicar após: 20240129000000_dnd_equipment_catalog.sql
-- ============================================================


-- ── 1. Permitir 'keep_lowest' em roll_mode ───────────────────
...
alter table public.dice_rolls
  add constraint dice_rolls_roll_mode_valid
    check (roll_mode in ('sum', 'keep_highest', 'keep_lowest'));


-- ── 2. Substituir trigger de validação (adiciona keep_lowest) ─
...
begin
  ...
  if new.roll_breakdown is not null then
    ...
    loop
      ...
      if v_type is null or v_type not in ('sum', 'keep_highest', 'keep_lowest', 'modifier') then
        raise exception 'Tipo inválido em roll_breakdown: %.', coalesce(v_type, 'null');
      end if;

      if v_type in ('sum', 'keep_highest', 'keep_lowest') then
        ...
        if v_type = 'sum' then
          ...
          -- keep_lowest: kept deve ser o menor valor de results
          v_kept := (v_item->>'kept')::integer;
    ...
    -- Sem roll_breakdown: keep_highest/keep_lowest exigem pelo menos 2 dados
    -- (com roll_breakdown, 1#d3 ou 1~d3 é permitido — manter o único resultado)
